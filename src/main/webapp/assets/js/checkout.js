...
const setAddressShipping = (state, country) => {
	if (country == null || country == "") {
		$("#shippingAddress").text("Address not found!");
	} else {
		if (state == null || state == "") {
			$("#shippingAddress").text(country)
			$("#countryData").val(country)
		} else {
			$("#shippingAddress").text(state + ", " + country)
			$("#stateData").val(state)
		}
	}
}
$(document).ready(function() {
	var currentCountry;
	var currentState;
	setAddressShipping(currentState, currentCountry)
	const countries = data.countries;
	countries.map((item, index) => {
		$("#countries-list").append(
			"<option value = '" + index + "'>" + item.country + "</option>"
		);
	});
	var stateArr
	$("#countries-list").change(function(e) {
		$("#states-list").empty();
		$("#states-list").append("<option>Select your countryside</option>");
		stateArr = countries[e.target.value].states;
		stateArr.map((item, index) => {
			$("#states-list").append(
				"<option value='" + index + "'>" + item + "</option>"
			);
		});
		currentCountry = countries[e.target.value].country;
		setAddressShipping(currentState, currentCountry)
	});
	$("#states-list").change(function(e) {
		currentState = stateArr[e.target.value];
		setAddressShipping(currentState, currentCountry)
	});
	$("#useDefaultAddress").click(function(e) {
		const address = $("#address").val()
		$("input[name=streetData]").val(address)
	});
	$("#checkoutForm").submit(function(e) {
		var country = $("#countries-list").val();
		var state = $("#states-list").val();
		var address = $("#streetInput").val();
		if (country == null || country == "" || state == null || state == "" ||
			address == null || address == "") {
			e.preventDefault()
			swal("Failed", "Cant post with empty field", "error")
		}
	});
	$("#confirmCheckoutForm").submit(function (e) {
		...
		const form = {
			country, state, street, fullname, phone, email, note, payment_method
		}
		e.preventDefault();
		$.ajax({
			type: "post",
