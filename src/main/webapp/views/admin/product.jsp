						...
						<div class="card-header border-0">
							...
							<div class="card-tools">
								<ul class="pagination pagination-sm">
									...
									<li class="page-item"><a href="{{ route('Adproduct') }}"
										class="page-link">1</a></li>
									<li class="page-item"><a
										href="{{ route('Adproduct') }}{{ '?page=2' }}"
										class="page-link">2</a></li>
									<li class="page-item"><a
										href="{{ route('Adproduct') }}{{ '?page=3' }}"
								...
								<tbody>
									...
									<tr>
										<td><img style="border: 0.5px solid black"
											src="{{ asset(" User/assets/images/product/$pro->image")
											}}" alt="Product" class="img-circle img-size-32 mr-2"> {{
											$pro->name}}</td>
										<td>{{ $pro->price }}</td>
										<td>{{ $pro->quantity }}</td>
										<td>{{ $pro->view }}</td>
										...
										<td>
											<form action="{{ route('deleteProduct') }}">
												<button type="submit" class="btn text-danger"
													id="deleteproduct" name="deleteproduct"
													value="{{ $pro->id }}">
									...
									<div class="modal" id="p{{ $pro->id }}">
										<div class="modal-dialog">
											<div class="modal-content">
												...
												<!-- Modal body -->
												<form action="{{ route('updateProduct', $pro->id) }}"
													method="POST">
													@csrf
													<div class="modal-body">
														...
														<div class="form-group">
															<label for="">Image</label> <br> <input type="file"
																id="imagepro" name="upimage" required />
														</div>
														<div class="form-group">
															<label for="">Description</label>
															<textarea class="form-control" name="updesc" id="updesc"
																required placeholder="Enter description" rows="5">{{ $pro->description }}
                            </textarea>
														</div>
														<div class="form-group">
														...
														<div class="form-group">
															<label for="">Status</label> <input type="text"
																class="form-control" name="upstatus"
																value="{{ $pro->status }}">
														</div>
														<div class="form-group">
															<label for="">Product details </label>
															<textarea class="form-control" name="updetail"
																id="updetails" required placeholder="Enter details"
																rows="5">{{ $pro->details }}
                            </textarea>
														</div>
														<div class="form-group">
															<label for="">Specifications</label>
															<textarea class="form-control" name="upspec" id="upspec"
																required placeholder="Enter specifications" rows="5">{{ $pro->specifications }}
                            </textarea>
														</div>
								...
								<form action="{{ route('insertProduct') }}" method="POST">
									...
									<div class="form-group">
										...
										<textarea class="form-control" name="description"
											id="description" required placeholder="Enter description"
											rows="5"></textarea>
									</div>
									...
									<div class="form-group">
										...
										<textarea class="form-control" name="detail" id="detail"
											required placeholder="Enter detailed for the product"
											rows="5"></textarea>
									</div>
									<div class="form-group">
										<label for="">Specifications</label><br>
										<textarea class="form-control" name="specifications"
											id="specifications" required
											placeholder="Enter specifications" rows="5"></textarea>
									</div>
