						...
						<div class="card-header">
							...
							<div class="card-tools">
								...
								<ul class="pagination pagination-sm">
									...
									<li class="page-item"><a href="{{ route('Adblog') }}"
										class="page-link">1</a></li>
									<li class="page-item"><a
										href="{{ route('Adblog') }}{{ '?page=2' }}" class="page-link">2</a></li>
							...
							<ul class="todo-list" data-widget="todo-list">
								...
								<li>
									...
									<div class="icheck-primary d-inline ml-2">
										--}}
										<form class="icheck-primary d-inline ml-2"
											action="{{ route('changeStatus', $blog->idblog) }}"
											id="checkstatus" method="POST">
											@csrf @if ($blog->status) <input type="checkbox" value=""
												name="todo1" id="todoCheck1" onchange=" submit();">
											<input type="hidden" name="idstatus" value="0"> @else
											<input type="checkbox" value="" name="todo1" id="todoCheck1"
												checked onchange=" submit();"> <input type="hidden"
												name="idstatus" value="1"> @endif <label
												for="todoCheck1"></label>
										</form>
										{{--
									</div> --}} <!-- todo text --> <span class="text"
									style="display: inline-block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 50%; vertical-align: top">{{
										$blog->title }}</span> <!-- Emphasis label --> <small
									class="badge badge-danger"><i class="far fa-clock"></i>
										{{ substr($blog->date, 0, 10) }}</small> <!-- General tools such as edit or delete-->
									<div class="tools">
										<form action="{{ route('deleteBlog') }}">
											<button type="submit" class="btn text-danger" id="deleteblog"
												name="deleteblog" value="{{ $blog->idblog }}">
								...
								<div class="modal" id="m{{ $blog->idblog }}">
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<!-- Modal body -->
											<form action="{{ route('updateBlog', $blog->idblog) }}"
												...
												<div class="modal-body">
													...
													<div class="form-group">
														<label for="">Title</label> <input type="text"
															class="form-control" name="uptitle" id="uptitle"
															value="{{ $blog->title }}">
													</div>
													<div class="form-group">
														<label for="">Author</label> <input type="text"
															class="form-control" name="upauthor"
															value="{{ $blog->author }}">
													</div>
													...
													<div class="form-group">
														<label for="">Image</label> <br> <input type="file"
															id="imageblog" accept=".jpg" name="upimage" required />
													</div>
													...
													<div class="form-group">
														<label for="">Image 1</label> <br> <input type="file"
															name="upimage1" id="upimage1" accept=".jpg" required />
													</div>
													<div class="form-group">
														<label for="">Image 2</label> <br> <input type="file"
															name="upimage2" id="upimage2" accept=".jpg" required />
													</div>
												</div>
								...
								<form action="{{ route('insertBlog') }}" method="POST">
										...
										<div class="card-body table-responsive p-0"
											...
											<div ng-app="ckeDemo">
												<div ng-controller="ckeController as cke" layout="column">
													<div ckeditor="cke.options" id="contentck"
														ng-model="cke.content" ready="cke.onReady()"></div>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<textarea class="form-control" name="content1" id="content1"
											required placeholder="Enter detailed content 1 for the blog"
											rows="5"></textarea>
									</div>
									<div class="form-group">
										<label for="">Content 2</label>
										<textarea class="form-control" name="content2" id="content2"
											required placeholder="Enter detailed content 2 for the blog"
											rows="5"></textarea>
									...
									<div class="form-group">
										...
										<textarea class="form-control" name="content3" id="conten3"
											required placeholder="Enter detailed content 3 for the blog"
											rows="5"></textarea>
									...
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												<label for="">Image 1</label> <br> <input type="file"
													name="image1" id="image1" accept=".jpg" required />
											</div>
										</div>
										<div class="col-md-6">
											<div class="form-group">
												<label for="">Image 2</label> <br> <input type="file"
													name="image2" id="image2" accept=".jpg" required />
											</div>
										</div>
									</div>
...
<script type="text/javascript"
	src="https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.5.3/angular-animate.min.js">
...
<script>
	$(document).ready(
			function() {
				...
				$('#insertblog').click(
						function() {
							var con = CKEDITOR.instances.contentck.document
									.getBody().getText();
							// alert(con);
							// $('#content').value = con;
